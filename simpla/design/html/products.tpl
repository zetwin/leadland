{* Вкладки *}
{capture name=tabs}
	<li class="active"><a href="{url module=ProductsAdmin keyword=null category_id=null store_id=null brand_id=null filter=null page=null}">Товары</a></li>
	...
	{if in_array('category_brands', $manager->permissions)}<li><a href="index.php?module=Category_brandsAdmin">Связка Категория+Бренд</a></li>{/if}
{/capture}
...
<div id="main_list">
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
			...
			<div class="name product_name cell">
			 ...
			 <ul>
					...
					<li {if !$variant@first}class="variant" style="display:none;"{/if}>
							...
							{foreach $currencies as $c}
								...
								{else}
									{if $currency->id == $c->id}
										selected
									{/if}
								{/if}  mnozhitel="{$c->rate_to/$c->rate_from}" value="{$c->id}">{$c->code}</option>
							{/foreach}
						</select>
						...
						<input  type="text" name="price[{$variant->id}]" value="{if $variant->currency}{if $variant->base_price}{$variant->base_price*$currencies[$variant->currency]->rate_to/$currencies[$variant->currency]->rate_from|escape}{else}{$variant->price*$currencies[$variant->currency]->rate_to/$currencies[$variant->currency]->rate_from|escape}{/if}{else}{$variant->price}{/if}"/>	
						</div>
		...
		<span id="change_price" style="display: none;">
			<select name="op_type"><option value="1">коэффициент</option><option value="0">прибавить/отнять</option></select> <input type="text" name="value" size="3" /> <label><input type="checkbox" name="save_old" value="1" title="сохранить старую цену" />сохранить старую цену</label>
		</span>
...
<div id="right_menu">
		...
		<!-- Категории товаров -->
	{function name=stores_tree}
	{if $stores}
	<ul>
		{if $stores[0]->parent_id == 0}
		<li {if !$store->id}class="selected"{/if}><a href="{url store_id=null brand_id=null}">Все поставщики</a></li>	
		{/if}
		{foreach $stores as $c}
		<li store_id="{$c->id}" {if $store->id == $c->id}class="selected"{else}class="droppable store"{/if}><a href='{url keyword=null brand_id=null page=null store_id={$c->id}}'>{$c->name}</a></li>
		{stores_tree stores=$c->substores}
		{/foreach}
	</ul>
	{/if}
	{/function}
	...
	<!-- Фильтры -->
	<ul>
		<li {if !$filter}class="selected"{/if}><a href="{url brand_id=null keyword=null page=null filter=null}">Все товары</a></li>
		<li {if $filter=='featured'}class="selected"{/if}><a href="{url keyword=null brand_id=null page=null filter='featured'}">Рекомендуемые</a></li>
		<li {if $filter=='discounted'}class="selected"{/if}><a href="{url keyword=null brand_id=null page=null filter='discounted'}">Со скидкой</a></li>
		<li {if $filter=='visible'}class="selected"{/if}><a href="{url keyword=null brand_id=null page=null filter='visible'}">Активные</a></li>
		<li {if $filter=='hidden'}class="selected"{/if}><a href="{url keyword=null brand_id=null page=null filter='hidden'}">Неактивные</a></li>
		<li {if $filter=='outofstock'}class="selected"{/if}><a href="{url keyword=null brand_id=null page=null filter='outofstock'}">Отсутствующие</a></li>
	</ul>
	<!-- Фильтры -->
