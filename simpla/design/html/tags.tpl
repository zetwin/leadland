...
<div id="header">
	...
	<form method="post">	<input type="hidden" name="session_id" value="{$smarty.session.id}"><input type="hidden" name="recount" value="1">  <input type="submit" value="Пересчитать теги"></form>
</div>	
<!-- Заголовок (The End) -->
...
<div id="main_list" class="categories">
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		...
		<div id="list" class="sortable">
			...
			{foreach $tags as $tag}
			<div class="{if !$tag->visible}invisible{/if} row">		
				<div class="tree_row">
					...
					<div class="cell">
						<a href="{url module=TagAdmin id=$tag->id return=$smarty.server.REQUEST_URI}">{$tag->name|escape} ({$tag->items_count|escape})</a> 	 			
					</div>
					<div class="icons cell">
						<a class="preview" title="Предпросмотр в новом окне" href="/tag/{$tag->url}" target="_blank"></a>				
