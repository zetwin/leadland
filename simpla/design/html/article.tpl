{capture name=tabs}
	<li class="active"><a href="{url module=ArticlesAdmin category_id=$article->category_id return=null brand_id=null id=null}">Статьи</a></li>
...
//////////////////////////////////
	...
	function show_category_features(category_id)
	{
		...
		$.ajax({
			...
			data: {category_id: category_id, article_id: $("input[name=id]").val()},
			dataType: 'json',
...
<div class="message message_success">
	<span class="text">{if $message_success=='added'}Статья добавлена{elseif $message_success=='updated'}Статья изменена{else}{$message_success|escape}{/if}</span>

	<a class="link" target="_blank" href="../article/{$article->url}">Открыть статью на сайте</a>
	{if $smarty.get.return}
	<a class="button" href="{$smarty.get.return}">Вернуться</a>
	{/if}
	
	<span class="share">		
		<a href="#" onClick='window.open("http://vkontakte.ru/share.php?url={$config->root_url|urlencode}/articles/{$article->url|urlencode}&title={$article->name|urlencode}&description={$article->annotation|urlencode}&image={$article_images.0->filename|resize:1000:1000|urlencode}&noparse=true","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
  		<img src="{$config->root_url}/simpla/design/images/vk_icon.png" /></a>
		<a href="#" onClick='window.open("http://www.facebook.com/sharer.php?u={$config->root_url|urlencode}/articles/{$article->url|urlencode}","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
  		<img src="{$config->root_url}/simpla/design/images/facebook_icon.png" /></a>
		<a href="#" onClick='window.open("http://twitter.com/share?text={$article->name|urlencode}&url={$config->root_url|urlencode}/articles/{$article->url|urlencode}&hashtags={$article->meta_keywords|replace:' ':''|urlencode}","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
...
<div class="message message_error">
	<span class="text">{if $message_error=='url_exists'}Статья с таким адресом уже существует{elseif $message_error=='empty_name'}Введите название{else}{$message_error|escape}{/if}</span>
	{if $smarty.get.return}
	<a class="button" href="{$smarty.get.return}">Вернуться</a>
	{/if}
</div>
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
 	...
 	<div id="name">
		...
		<div class="checkbox">
			<input name=visible value='1' type="checkbox" id="active_checkbox" {if $article->visible}checked{/if}/> <label for="active_checkbox">Активен</label>
		</div>
		<div class="checkbox">
			<input name=featured value="1" type="checkbox" id="featured_checkbox" {if $article->featured}checked{/if}/> <label for="featured_checkbox">Рекомендуемый</label>
		</div>
	</div> 
	
		
	<div id="product_categories" {if !$articlecategories}style='display:none;'{/if}>
		<label>Категория</label>
		<div>
			<ul>
				{foreach name=articlecategories from=$article_categories item=article_category}
				<li>
					<select name="articlecategories[]">
						{function name=articlecategory_select level=0}
						{foreach from=$articlecategories item=articlecategory}
								<option value='{$articlecategory->id}' {if $articlecategory->id == $selected_id}selected{/if} articlecategory_name='{$articlecategory->name|escape}'>{section name=sp loop=$level}&nbsp;&nbsp;&nbsp;&nbsp;{/section}{$articlecategory->name|escape}</option>
								{articlecategory_select articlecategories=$articlecategory->subarticlecategories selected_id=$selected_id  level=$level+1}
						{/foreach}
						{/function}
						{articlecategory_select articlecategories=$articlecategories selected_id=$article_category->id}
					</select>
					<span {if not $smarty.foreach.articlecategories.first}style='display:none;'{/if} class="add"><i class="dash_link">Дополнительная категория</i></span>
					<span {if $smarty.foreach.articlecategories.first}style='display:none;'{/if} class="delete"><i class="dash_link">Удалить</i></span>
				</li>
				{/foreach}		
			</ul>
		</div>
	...
	<div id="column_left">
		...
		<div class="block layer">
			...
			<ul>
				<li><label class=property>Адрес</label><div class="page_url"> /articles/</div><input name="url" class="page_url" type="text" value="{$article->url|escape}" /></li>
				<li><label class=property>Заголовок</label><input name="meta_title" class="simpla_inp" type="text" value="{$article->meta_title|escape}" /></li>
				<li><label class=property>Ключевые слова</label><input name="meta_keywords" class="simpla_inp" type="text" value="{$article->meta_keywords|escape}" /></li>
				<li><label class=property>Описание</label><textarea name="meta_description" class="simpla_inp" />{$article->meta_description|escape}</textarea></li>
			</ul>
		</div>
	...
	<div id="column_right">
		...
		<div class="block layer">
			<h2>Связанная категория товаров</h2>
						<select name="related_category">
				<option value="" selected>
				
						{function name=category_select level=0}
						{foreach from=$categories item=category}
								<option value='{$category->id}' {if $category->id == $article->related_category}selected{/if} category_name='{$category->name|escape}'>{section name=sp loop=$level}&nbsp;&nbsp;&nbsp;&nbsp;{/section}{$category->name|escape}</option>
		...
		<label>Связанный Бренд</label>
		<select name="brand_id">
            <option value='0' {if !$article->brand_id}selected{/if} brand_name=''>Не указан</option>
       		{foreach from=$brands item=brand}
            	<option value='{$brand->id}' {if $article->brand_id == $brand->id}selected{/if} brand_name='{$brand->name|escape}'>{$brand->name|escape}</option>
        	{/foreach}
		</select>
	</div>
...
<input class="button_green button_save" type="submit" name="" value="Сохранить" />		
	...
	<div class="block">		
		...
		<textarea name="body" class="editor_large">{$article->body|escape}</textarea>
	</div>
