...
<div class="message message_success">
	<span class="text">{if $message_success=='added'}Тег добавлен{elseif $message_success=='updated'}Тег обновлен{else}{$message_success}{/if}</span>
	<a class="link" target="_blank" href="/tag/{$tag->url}">Открыть тег на сайте</a>
...
<div class="message message_error">
	<span class="text">{if $message_error=='url_exists'}Тег с таким адресом уже существует{else}{$message_error}{/if}</span>
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	<div id="name">
		...
		<div class="checkbox">
			<input name=visible value='1' type="checkbox" id="active_checkbox" {if $tag->visible}checked{/if}/> <label for="active_checkbox">Активна</label>
		</div>
	</div> 
	...
	<div id="column_left">
		...
		<div class="block layer">
			...
			<ul>
				<li><label class=property>Адрес</label><div class="page_url">/catalog/</div><input name="url" class="page_url" type="text" value="{$tag->url|escape}" /></li>
				<li><label class=property>Заголовок</label><input name="meta_title" class="simpla_inp" type="text" value="{$tag->meta_title|escape}" /></li>
				<li><label class=property>Описание</label><textarea name="meta_description" class="simpla_inp" />{$tag->meta_description|escape}</textarea></li>
			</ul>
		</div>
