		...
		<div id="list" class="sortable">
			...
			<div class="{if !$comment->approved}unapproved{/if} row">
				...
				<div class="name cell">
					...
					к посту <a target="_blank" href="{$config->root_url}/blog/{$comment->post->url}#comment_{$comment->id}">{$comment->post->name}</a>
					{elseif $comment->type == 'page'}
					к странице <a target="_blank" href="{$config->root_url}/{$comment->page->url}#comment_{$comment->id}">{$comment->page->name}</a>
					{/if}
...
<div id="right_menu">
	...
	<ul>
		...
		<li {if $type == 'page'}class="selected"{/if}><a href='{url keyword=null type=page}'>К страницам</a></li>
	</ul>
