{capture name=tabs}
	...
	{if in_array('stores', $manager->permissions)}<li><a href="index.php?module=StoresAdmin">Поставщики</a></li>{/if}
...
$(function() {
	...
	$("#product_categories .delete").live('click', function() {
		...
		// Добавление категории
	$('#product_stores .add').click(function() {
		$("#product_stores ul li:last").clone(false).appendTo('#product_stores ul').fadeIn('slow').find("select[name*=stores]:last").focus();
		$("#product_stores ul li:last span.add").hide();
		$("#product_stores ul li:last span.delete").show();
		return false;		
	});

	// Удаление категории
	$("#product_stores .delete").live('click', function() {
		$(this).closest("li").fadeOut(200, function() { $(this).remove(); });
		return false;
	});
	...
	$(".instructions ul").sortable({ tolerance: 'pointer'});

	// Удаление документов
	$(".instructions a.delete").live('click', function() {
		 $(this).closest("li").fadeOut(200, function() { $(this).remove(); });
		 return false;
	});
	// Загрузить документы с компьютера
	$('#upload_instruction').click(function() {
		$("<input class='upload_instruction' name=instructions[] type=file multiple>").appendTo('div#add_instruction').focus().click();
	});
	// Или с URL
	$('#add_instruction_url').click(function() {
		$("<input class='remote_instruction' name=instructions_urls[] type=text value='http://'>").appendTo('div#add_instruction').focus().select();
	});
	// Или перетаскиванием
	if(window.File && window.FileReader && window.FileList)
	{
		$("#dropZoneInstructions").show();
		$("#dropZoneInstructions").on('dragover', function (e){
			$(this).css('border', '1px solid #8cbf32');
		});
		$(document).on('dragenter', function (e){
			$("#dropZoneInstructions").css('border', '1px dotted #8cbf32').css('background-color', '#c5ff8d');
		});
	
		dropInput = $('.dropInputinstructions').last().clone();
		
		function handleFileSelectinstructions(evt){
			var files = evt.target.files; // FileList object
			// Loop through the FileList and render image files as thumbnails.
		    for (var i = 0, f; f = files[i]; i++) {
			...
			reader.onload = (function(theFile) {
				return function(e) {
					// Render thumbnail.
					$("<li class=wizard><input name=instructions_urls[] type=hidden value='"+theFile.name+"'>"+theFile.name+" <a href='' class='delete'></a></li>").appendTo('div .instructions ul');
					temp_input =  dropInput.clone();
					$('.dropInputinstructions').hide();
					$('#dropZoneInstructions').append(temp_input);
					$("#dropZoneInstructions").css('border', '1px solid #d0d0d0').css('background-color', '#ffffff');
					clone_input.show();
		        };
		      })(f);
		...
		$('.dropInputinstructions').live("change", handleFileSelectinstructions);
	};
	//Документация товара конец
...
//////////////////////////////////
		...
		$.ajax({
			...
			success: function(data){
				for(i=0; i<data.length; i++)
				{
					...
					subline = $("<label class=property></label><span><input class='simpla_inp' type='text'/> <span class='delete'></span></span>");
                    ...
                    if(feature.value.length>1){
                        for(var j=1;j<feature.value.length;j++){
                            var new_subline = subline.clone(true);
	...
	$('ul.prop_ul input[name*=options]').live('focus', function(index) {
...
$('.prop_ul .delete').live('click', function() {
		...
		$.ajax({
 			 url: "ajax/get_images.php",
  				...
  				success: function(data){
    				...
    				{
	    				image_url = data[i];
						$("<li class=wizard><a href='' class='delete'></a><a href='"+image_url+"' target=_blank><img onerror='$(this).closest(\"li\").remove();' src='"+image_url+"' /><input name=images_urls[] type=hidden value='"+image_url+"'></a></li>").appendTo('div .images ul');
    				}
	...
	$('#images_wizard_title').click(function() {
		
		$('#images_wizard img').attr('src', 'design/images/loader.gif');
		if(name_changed)
			$('div.images ul li.wizard').remove();
		name_changed = false;
		key = $('input[name=name]').val()  + " " + $('select[name="brand_id"] option:selected').attr('brand_name');
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	...
	<div id="column_left">
		...
		<div class="block layer">
			...
			<ul>
				...
				<li><label class=property>Демо папка</label><input name="demo" class="simpla_inp" type="text" value="{$product->demo|escape}" /></li>
		...
		<div class="block layer">
			<h2>Теги</h2>
				<input type="text" name="tags" value="{foreach $product->tags as $tag}{$tag->name|escape}{if !$tag@last},{/if}{/foreach}" />
            {literal}
            <script>
            $('input[name=tags]').inputosaurus({
                // width : '350px',
								autoCompleteSource : [
								{/literal}
								{foreach $alltags as $tag}'{$tag->name|escape}'{if !$tag@last},{/if}{/foreach}
								{literal}],
		...
		<div class="block layer">
		<h2>Поставщики</h2>
		<div id="product_stores" {if !$stores}style='display:none;'{/if}>
		<div>
			<ul>
				{foreach name=stores from=$product_stores item=product_store}
				<li>
					<select name="stores[id][]" style="    width: 190px;    margin-right: 3px;">
					<option value='' {if !$product_store->id}selected{/if}>Свой склад</option>
						{function name=store_select level=0}
						{foreach from=$stores item=store}
								<option value='{$store->id}' {if $store->id == $selected_id}selected{/if} store_name='{$store->name|escape}'>{section name=sp loop=$level}&nbsp;&nbsp;&nbsp;&nbsp;{/section}{$store->name|escape}</option>
								{store_select stores=$store->substores selected_id=$selected_id  level=$level+1}
						{/foreach}
						{/function}
						{store_select stores=$stores selected_id=$product_store->id}
					</select>
					<span class="icons cell" style="    margin-right: 6px;"><a class="preview"   title="На сайте Поставщика" href="https://anon.click/?{$product_store->url|escape}" target="_blank"></a></span>
					<input name="stores[url][]" class="simpla_inp" type="text" value="{$product_store->url|escape}"  style="    width: 190px;    margin-right: 3px;"/>
					<span {if not $smarty.foreach.stores.first}style='display:none;'{/if} class="add"><i class="dash_link"></i></span>
					<span {if $smarty.foreach.stores.first}style='display:none;'{/if} class="delete"><i class="dash_link"></i></span>
				</li>
				{/foreach}		
			</ul>
		</div>
	...
	<div id="column_right">
		...
		<div class="block layer images">
			...
			<a href="#" id=images_wizard_title><img src="design/images/wand.png" alt="Подобрать автоматически по title" title="Подобрать автоматически по title"/></a>
			</h2>
		...
		<!-- Документация товара -->	
		<div class="block layer instructions">
			<h2>Документация товара</h2>
			<ul>{foreach from=$product_instructions item=instruction}<li>
					<a target="_blank" href="../{$config->original_instructions_dir}{$instruction->filename}"><img src="design/images/filetypes/new-window.png"></a>
					<input name='names[]' value='{if $instruction->name}{$instruction->name}{else}{$instruction->filename}{/if}'>
					<input type=hidden name='instructions[]' value='{$instruction->id}'>
					<span>{$instruction->filename}</span>
					<a href='#' class="delete"><img src='design/images/cross-circle-frame.png'></a>
				</li>{/foreach}</ul>
			<div id=dropZoneInstructions>
				<div id=dropMessageInstructions>Перетащите файлы сюда</div>
				<input type="file" name="dropped_instructions[]" multiple class="dropInputinstructions">
			</div>
			<div id="add_instruction"></div>
			<span class=upload_instruction><i class="dash_link" id="upload_instruction">Добавить документы</i></span> или <span class=add_instruction_url><i class="dash_link" id="add_instruction_url">загрузить из интернета</i></span>
		</div>
		
		<!-- Видео товара -->	
		<div class="block layer">
			<h2>Видео</h2>
			<label class=property>Код вставки</label>
			<textarea rows="5" style="width:100%;" name="video" class="simpla_inp" />{$product->video|escape}</textarea>
			</div>
	...
	<h2>Варианты и размеры</h2>
	
	<label class=property>Шаблон вариаций</label><input name="variants_names" class="simpla_inp" type="text" value="{$product->variants_names|escape}" />
		<ul id="header">
			<li class="variant_move"></li>
			<li class="variant_img"></li>
			
			<li class="variant_name">Наименование варианта</li>    
			<li class="variant_sku">Артикул</li>	
			<li class="variant_price">Цена в валюте</li>	
			<li class="variant_supplier_price">Цена поставщика</li>
			<li class="variant_discount">Старая цена</li>	
			...
			<li class="variant_unit">На складе</li>
			<li class="variant_amount">ед. изм.</li>
			<li class="variant_download">f</li>
			<li class="variant_delete"></li>
		...
		<ul>
			...
			<li class="variant_img"><a href='#' class="add_color"><img src="design/images/picture{if !$variant->images_ids}_empty{/if}.png" title="Привязать изображение" /></a> <input name="variants[images_ids][]" type="hidden" value="{$variant->images_ids|escape}" /></li>
			
			
			<li class="variant_name"><input name="variants[id][]" type="hidden" value="{$variant->id|escape}" /><input name="variants[name][]" type="" value="{$variant->name|escape}" /> </li>
			
			<li class="variant_sku">       <input name="variants[sku][]"           type="text"   value="{$variant->sku|escape}" /></li>
			<li class="variant_price"><input name="variants[base_price][]"         type="text"   value="{if $variant->base_price}{$variant->base_price|escape}{else}{$variant->price|escape}{/if}" /></li>
			<li class="variant_supplier_price"><input name="variants[supplier_price][]" type="text" value="{$variant->supplier_price|escape}" /></li>
			...
			<input name="variants[stock][]"         type="text"   value="{if $variant->infinity || $variant->stock == ''}∞{else}{$variant->stock|escape}{/if}" />
			</li>
			
			<li class="variant_unit">{if $units|count > 1}<select name="variants[unit][]" style="width: 40px;margin: 0;">{foreach $units as $key=>$unit}<option value="{$key}"{if $variant->unit == $key} selected{/if}>{$key}</option>{/foreach}</select>{else}{$settings->units}{/if}</li>
			...
			<li class="variant_delete"><a class="del_variant" href=""><img src="design/images/cross-circle-frame.png" alt="" /></a></li>
		</ul>
		{/foreach}		
		</div>
		<ul id=new_variant style='display:none;'>
			...
			<li class="variant_name"><input name="variants[id][]" type="hidden" value="" /><input name="variants[name][]" type="" value="" /></li>
			
			<li class="variant_sku"><input name="variants[sku][]" type="" value="" /></li>
			<li class="variant_price"><input name="variants[base_price][]"         type="text"   value="{if $variant->base_price}{$variant->base_price|escape}{else}{$variant->price|escape}{/if}" /></li>
			<li class="variant_supplier_price"><input name="variants[supplier_price][]" type="text" value="" /></li>
			<li class="variant_discount">  <input name="variants[base_compare_price][]" type="text"   value="{if $variant->base_compare_price}{$variant->base_compare_price|escape}{else}{$variant->compare_price|escape}{/if}" /></li>
			<li class="variant_currency">
				<select type="text" name="variants[currency][]">
					{foreach $currencies as $c}
			...
			<li class="variant_amount">
			<input name="variants[stock][]" type="" value="∞" /> 
			</li>
			
			<li class="variant_unit">{if $units|count > 1}<select name="variants[unit][]" style="width: 40px;margin: 0;">{foreach $units as $key=>$unit}<option value="{$key}">{$key}</option>{/foreach}</select>{else}{$settings->units}{/if}</li>
