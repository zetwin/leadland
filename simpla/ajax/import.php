...
{	
	...
	private $columns_names = array(
			'name'=>             array('product', 'name', 'товар', 'название', 'наименование' , 'Товар' ),
			'url'=>              array('url', 'адрес'),
...
if(isset($item['compare_price']))
	...
	if ($currency)
		...
		if(!empty($variant_id) && !empty($product_id))
		{
			...
			if(!empty($stores_ids)){
			$this->stores->delete_product_stores($product_id);
			...
			$store_urls[] = null;
				...
				$store_urls[$i] = '';
					...
					$this->stores->add_product_store($product_id, $c_id, $store_urls[$i]);
				...
				foreach($tags as $tag)
				{
					if(!empty($tag))
					{
						...
						if(empty($tag_id)){
							...
							$this->tags->add_tag_item('product',$product_id,$tag_id);
					}
				}
			}
