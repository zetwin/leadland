...
class Categories extends Simpla
{
	...
	public function get_categories($filter = array())
	{
		...
		// print_r($this->all_categories);
		return $this->all_categories;
	}	
	
		// Функция возвращает массив категорий
	public function get_random_categories($filter = array())
	{
		// if(!isset($this->categories_tree))
			// $this->init_categories();
		...
		{
	// print_r($filter['limit']);
			$query = $this->db->placehold("SELECT * FROM __categories  ORDER BY RAND() LIMIT ?", $filter['limit']);
			$this->db->query($query);
...
// {
	// $query = $this->db->placehold("SELECT id,url,parent_id FROM __categories WHERE visible=1 ORDER BY position");
    	// $this->db->query($query);
...
//Меняем вид URL	
	...
	public function add_category($category, $copy_features = false)
	{
		...
		$this->db->query("INSERT INTO __categories SET ?%, last_update=NOW()", $category);
		$id = $this->db->insert_id();
		$this->db->query("UPDATE __categories SET position=id, last_update=NOW() WHERE id=?", $id);	
	...
	{
		$query = $this->db->placehold("UPDATE __categories SET ?%, last_update=NOW() WHERE id=? LIMIT 1", $category, intval($id));
		$this->db->query($query);
		unset($this->categories_tree);			
		unset($this->all_categories);	
		return intval($id);
	...
	private function init_categories()
	{
		...
		$query = $this->db->placehold("SELECT c.id, c.parent_id, c.name, c.description, c.url, c.meta_title, c.meta_keywords, c.meta_description, c.image, c.visible, c.position, c.last_update
