...
class Design extends Simpla
{
	...
	public function __construct()
	{
		...
		$this->smarty->registerPlugin('modifier', 'instruction', array($this, 'instruction_modifier'));	
		$this->smarty->registerPlugin('modifier', 'resizearticle', array($this, 'resizearticle_modifier'));
...
$this->assign('settings', $settings);
	...
	public function resize_modifier($filename, $width=0, $height=0, $set_watermark=false, $smart=false)
	{
		$resized_filename = $this->image->add_resize_params($filename, $width, $height, $set_watermark, $smart);
		$resized_filename_encoded = $resized_filename;
		...
		public function resizearticle_modifier($filename, $width=0, $height=0, $set_watermark=false, $smart=false)
	{
		$resized_filename = $this->imagearticle->add_resize_params($filename, $width, $height, $set_watermark, $smart);
		...
		$resized_filename_encoded = rawurlencode($resized_filename_encoded);
				// print_r($resized_filename_encoded);
		return $this->config->root_url.'/'.$this->config->resized_articleimages_dir.$resized_filename_encoded/*.'?'.$this->config->token($resized_filename)*/;

	}
	
	public function instruction_modifier($filename, $width=0, $height=0, $set_watermark=false)
	{
		$resized_filename = $this->instruction->add_resize_params($filename, $width, $height, $set_watermark);
		$resized_filename_encoded = $resized_filename;
		...
		return $this->config->root_url.'/'.$this->config->resized_instructions_dir.$resized_filename_encoded/*.'?'.$this->config->token($resized_filename)*/;
	}
