...
class Variants extends Simpla
{
	...
	*/
	...
	{		
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, v.supplier_price, v.base_price as base_price, NULLIF(v.compare_price, 0) as compare_price, NULLIF(v.base_compare_price, 0) as base_compare_price, v.currency as currency, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.unit, v.name, v.images_ids, v.attachment, v.attachment_size, v.position
	...
	{	
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, v.supplier_price, v.base_price as base_price, NULLIF(v.compare_price, 0) as compare_price, NULLIF(v.base_compare_price, 0) as base_compare_price, v.currency as currency, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.unit, v.name, v.images_ids, v.attachment, v.attachment_size
