...
class Image extends Simpla
{
	...
	function resize($filename)
	{
		...
		$size = ($width?$width:0).'x'.($height?$height:0).($set_watermark?"w":'').($smart?$smart:'');
        $image_sizes = array();
	...
	public function add_resize_params($filename, $width=0, $height=0, $set_watermark=false, $smart=false)
	{
		...
		if($width>0 || $height>0)
			$resized_filename = $file.'.'.($width>0?$width:'').'x'.($height>0?$height:'').($set_watermark?'w':'').($smart?$smart:'').'.'.$ext;
		else
	...
	public function get_resize_params($filename)
	{
		...
		if(!preg_match('/(.+)\.([0-9]*)x([0-9]*)(w)?(s|st)?\.([^\.]+)$/', $filename, $matches))
			return false;
		...
		$smart = $matches[5];	// ставить ли водяной знак
	...
	private function image_constrain_gd($src_file, $dst_file, $max_w, $max_h, $watermark=null, $watermark_offet_x=0, $watermark_offet_y=0, $watermark_opacity=1, $smart=null)
	{
		...
		// Если устаовлен флажок s или st для умной обрезки обрезаем SMART CROP s - по середине, st - сверху изображения
...
imagesavealpha($dst_img, true);
    ...
    imagealphablending($overlay, true);
		...
		case 'image/png':
			...
			return imagepng( $dst_img, $dst_file, 6, PNG_ALL_FILTERS);
		default:
			return false;
		}
