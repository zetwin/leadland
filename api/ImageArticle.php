...
class ImageArticle extends Simpla
{
	 ...
	 */
	function resizearticle($filename)
	{
		list($source_file, $width , $height, $set_watermark, $smart) = $this->get_resize_params($filename);
		...
		{	
			...
			$resized_file = $this->add_resize_params($original_file, $width, $height, $set_watermark, $smart);			
		}	
		else
		{
		...
		$originals_dir = $this->config->root_dir.$this->config->original_articleimages_dir;
		$preview_dir = $this->config->root_dir.$this->config->resized_articleimages_dir;
		...
		if(class_exists('Imagick') && $this->config->use_imagick)
			$this->image_constrain_imagick($originals_dir.$original_file, $preview_dir.$resized_file, $width, $height, $watermark, $watermark_offet_x, $watermark_offet_y, $watermark_transparency, $sharpen, $smart);
		else
			$this->image_constrain_gd($originals_dir.$original_file, $preview_dir.$resized_file, $width, $height, $watermark, $watermark_offet_x, $watermark_offet_y, $watermark_transparency, $smart);
		
		return $preview_dir.$resized_file;
	}

	public function add_resize_params($filename, $width=0, $height=0, $set_watermark=false, $smart=false)
	{
	...
	// print_r($filename, true);
		...
		if($width>0 || $height>0)
			$resized_filename = $file.'.'.($width>0?$width:'').'x'.($height>0?$height:'').($set_watermark?'w':'').($smart?'s':'').'.'.$ext;
		else
	...
	public function get_resize_params($filename)
	{
		...
		if(!preg_match('/(.+)\.([0-9]*)x([0-9]*)(w)?(s)?\.([^\.]+)$/', $filename, $matches))
			return false;
		...
		$smart = $matches[5] == 's';	// ставить ли водяной знак
		$ext = $matches[6];						// расширение файла
			
		return array($file.'.'.$ext, $width, $height, $set_watermark, $smart);
	}
	...
	public function download_image($filename)
	{
		...
		$this->db->query('SELECT 1 FROM __imagesarticle WHERE filename=? LIMIT 1', $filename);
		if(!$this->db->result())
			return false;
		...
		while(file_exists($this->config->root_dir.$this->config->original_articleimages_dir.$new_name))
		{
		...
		$this->db->query('UPDATE __imagesarticle SET filename=? WHERE filename=?', $new_name, $filename);
		
		// Перед долгим копированием займем это имя
		fclose(fopen($this->config->root_dir.$this->config->original_articleimages_dir.$new_name, 'w'));
		copy($filename, $this->config->root_dir.$this->config->original_articleimages_dir.$new_name);
		return $new_name;
	}
	...
	public function upload_image($filename, $name)
	{
		...
		{			
			...
			if(move_uploaded_file($filename, $this->config->root_dir.$this->config->original_articleimages_dir.$new_name))			
				return $new_name;
		}
	 ...
	 * @return bool
	 */
	private function image_constrain_gd($src_file, $dst_file, $max_w, $max_h, $watermark=null, $watermark_offet_x=0, $watermark_offet_y=0, $watermark_opacity=1, $smart=null)
	{
		$quality = 100;		
		...
		// Нужно ли обрезать?
		if (!$watermark && !$smart && ($src_w <= $max_w) && ($src_h <= $max_h))
	    {
		...
		// Если устаовлен флажок s для умной обрезки обрезаем SMART CROP
	if(!empty($smart) && !empty($max_w) && !empty($max_h)){
		...
		if ($source_aspect_ratio > $desired_aspect_ratio) {
				 ...
				 */
				$temp_height = $max_h;
				$temp_width = ( int ) ($max_h * $source_aspect_ratio);
		} else {
				/*
				 * Triggered otherwise (i.e. source image is similar or taller)
				 */
				$temp_width = $max_w;
				$temp_height = ( int ) ($max_w / $source_aspect_ratio);
		}
...
$x0 = ($temp_width - $max_w) / 2;
$y0 = ($temp_height - $max_h) / 2;
