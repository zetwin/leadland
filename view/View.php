...
class View extends Simpla
{
	...
	public function __construct()
	{
		...
		else
		{
           ...
           //$this->design->assign('themes',    $this->design->get_themes());
			...
			$this->design->smarty->registerPlugin("function", "get_new_articles",        array($this, 'get_new_articles_plugin'));
	...
	public function get_new_articles_plugin($params, &$smarty)
	{
		if(!isset($params['visible']))
			$params['visible'] = 1;
		...
		if(!empty($params['var']))
		{
			foreach($this->articles->get_articles($params) as $p)
				$articles[$p->id] = $p;
    ...
    if(!isset($params['visible']))
		...
		if (!empty($params['category_id']) && !empty($category_url))
		{
