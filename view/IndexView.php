...
{	
	...
	function fetch()
	{
		...
		$this->design->assign('articlecategories', $this->articlecategories->get_articlecategories_tree());
 ...
 //Меняем вид URL
     // $url = $this->urls[0];
	// if(!empty($url)){
	// $product = $this->products->get_product($url);
	// if($product && ($product->full_url == $this->module_url || $product->full_url.'/' == $this->module_url))
		// $module = 'ProductView';
  	// elseif($cat = $this->categories->get_category($url)||$b = $this->brands->get_brand($url))  
		// $module = 'ProductsView';
	// else
		// $module = 'PageView';
	// }
